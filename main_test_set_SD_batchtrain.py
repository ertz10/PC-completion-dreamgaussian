...
if __name__ == "__main__":
    ...
    # test objects
    ########## SHOE ###############
    cmd = "python main_sd_refine.py --config configs/text.yaml "
    #prompt = "prompt=\"a beige winter shoe with beige shoelaces, dark brown shoe sole, natural colors\" "
    prompt = "prompt=\"a DSLR foto of a beige winter shoe, dark brown shoe sole, natural colors, desaturated\" "
    save_path = "save_path=shoe "
    object_conf = "--object_conf=data/shoe/conf.yaml "
    load = "load=logs/shoe_model.ply "
    load_original = "load_original=data/shoe/shoe_cropped.ply "

    # send command
    command = cmd + prompt + save_path + object_conf + load + load_original  
    if shoe:
        p = Popen(command)
        p.communicate()

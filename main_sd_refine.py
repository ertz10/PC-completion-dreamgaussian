...
class GUI:
    def __init__(self, opt, opt_object):
        ...
        if self.opt.load is not None:
            ...
            self.renderer.initialize_static_and_dynamic(self.opt.load, self.opt.load_original, spatial_lr_scale=1)
        else:
            # CUSTOM CODE
            if self.opt.point_cloud is None:
                # initialize gaussians to a blob
                self.renderer.initialize(num_pts=self.opt.num_pts, spatial_lr_scale=1)
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            render_resolution = self.opt_object.sd_render_res #if step_ratio < 0.3 else (512 if step_ratio < 0.6 else 512)
            images = []
            AABBimages = []
            static_images = []
            dynamic_images = []
            ...
            for _ in range(self.opt.batch_size):
                ...
                if (self.step % 3 == 0 and self.opt.only_dynamic_splats == False):
                    ...
                    if (angle1 > angle2): # e.g. [280, 30] going around zero 
                        ...
                        hor = rand1 if decider == 0 else rand2
                    else:
                        #hor = int(np.random.randint(angle1, angle2) - 180.0)
                        hor = int(np.random.randint(angle1, angle2))
                else:
                ...
                out = self.renderer.render(cur_cam, bg_color=bg_color, only_dynamic_splats=self.opt.only_dynamic_splats)
                ...
                out_debug = self.renderer.render(cur_cam_debug, bg_color=torch.tensor([0,0,0], dtype=torch.float32, device='cuda'), only_dynamic_splats=self.opt.only_dynamic_splats)
                ##############
                # DEBUG render end
                ...
                if self.opt.mvdream or self.opt.imagedream:
                    for view_i in range(1, 4):
                        ...
                        #else:
                            # ONLY FOR DEBUG PURPOSE TODO remove in final version
                        out_i = self.renderer.render(cur_cam_i, bg_color=bg_color, only_dynamic_splats=self.opt.only_dynamic_splats)
                        ...
                        ######################################################################
                        with torch.no_grad():
                            static_points_image, dynamic_points_image, static_points_depth, dynamic_points_depth, static_points_alpha, dynamic_points_alpha = self.customLoss.GSRendererDepthBlending(self.renderer.gaussians, cur_cam_i, bg_color=bg_color, only_dynamic_splats=self.opt.only_dynamic_splats)
                            static_images.append(torch.vstack((static_points_image, static_points_alpha)))
                            dynamic_images.append(torch.vstack((dynamic_points_image, dynamic_points_alpha)))
                            static_depth_images.append(static_points_depth)
                            dynamic_depth_images.append(dynamic_points_depth)
            ...
            if self.enable_sd:
                if self.opt.mvdream or self.opt.imagedream:
                    ...
                    loss = loss + self.opt.lambda_sd * self.guidance_sd.train_step(images, out_debug["image"].unsqueeze(0), poses, self.customLoss, step_ratio=step_ratio if self.opt.anneal_timestep else None, 
                                                                                                                                                                                ...
                                                                                                                                                                                only_dynamic_splats=self.opt.only_dynamic_splats)
                else:
                    loss = loss + self.opt.lambda_sd * self.guidance_sd.train_step(images, out_debug["image"].unsqueeze(0), step_ratio=step_ratio if self.opt.anneal_timestep else None, customLoss=self.customLoss,
                                                                                                                                                        dynamic_images=dynamic_images, static_images=static_images,
                                                                                                                                                        dynamic_depth_images=dynamic_depth_images, static_depth_images=static_depth_images, current_cam_hors=hors, 
                                                                                                                                                        captured_angles_hor=self.captured_angles_hor, object_params=self.opt_object, only_dynamic_splats=self.opt.only_dynamic_splats)
    ...
    def save_model(self, mode='geo', texture_size=1024, index=0):
        ...
        if mode == 'geo':
            ...
            path = os.path.join(self.opt.outdir, self.opt.save_path + '_sd_refine_model.ply')
...
if __name__ == "__main__":
    ...
    if opt.gui:
        ...
        gui.train(opt_object.max_steps_refine)
